...
void semantic_error_string(int, int, char*);
...
A_TYPE *sem_expression(A_NODE *node)
{
	...
	switch(node->name) {
		case N_EXP_IDENT : 
			...
			switch (id->kind) { 
				...
				case ID_ENUM_LITERAL:
					...
					break;
				default: 
					semantic_error_string(38, node->line, id->name);
					break;
			}
			break;
		...
		case N_EXP_GEQ :
			...
			t2=sem_expression(node->rlink);
			if (isArithmeticType(t1) && isArithmeticType(t2))
				t=convertUsualBinaryConversion(node);  // here
			else if (!isCompatiblePointerType(t1,t2))
...
int sem_declaration(A_ID *id,int addr)
{
	...
	switch (id->kind) {
		...
		case ID_TYPE:
			...
			break; 
		default:
			semantic_error_string(89,id->line,id->name);
			break; 
	}
	return (size);
}
	...
	switch(node->name) {
		case N_EXP_IDENT : 
			...
			if (id->kind!=ID_ENUM_LITERAL) 
				semantic_error_string(19,node->line,id->name);
			else {
				result.type=int_type;
...
void semantic_error_string(int i, int ll, char *s) {
	semantic_err++;
