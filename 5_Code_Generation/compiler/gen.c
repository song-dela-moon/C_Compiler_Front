	...
	switch (id->kind) {
		...
		case ID_FUNC:
			if (id->type->expr) {
				...
				gen_statement(id->type->expr,0,0,0,0); // switch case 제외를 위해 (0,0) 으로?
				gen_code_i(RET,0,0);
			}
			break;
		case ID_PARM:
...
void gen_error(int i, int ll, char *s )
{
	...
	switch (i) {
		...
		case 100:
			...
			break;
		case 101:
			printf("Struct is not supported in this code generator \n");
			break;
		case 102:
			printf("Switch & Case are not supported in this code generator \n");
			break;
		default:
			printf("unknown \n");
			break;
